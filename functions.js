...
function transformarEnEditable(nodo) {
  ...
  if (editando == false) {
    ...
    var nuevoCodigoHtml = '<td><input type="text" name="alimento" id="alimento" value="' + alimento + '" size="10"></td>' +
      ...
      '<td><select name=ok id=ok><option value="1">Bien</option><option value="2">Mal</option></select></td>' +
      "<td>En edición</td>";
    nodoTr.innerHTML = nuevoCodigoHtml;
    nodoContenedorForm.innerHTML = "Pulse Aceptar para guardar los cambios o cancelar para anularlos" +
      '<form name = "formulario" action="recivido.html" method="get" onsubmit="capturarEnvio()" onreset="anular()">' +
